int rgbCalibrationVals[4][4]; // Each row contains measurements for a different block, each column is a given set of conditions 
int testCases[4][3] = {{0, 0, 0},{255, 0 ,0}, {0, 255, 0},{0, 0, 255}};
...
void colourId(){
  ...
  for (int i = 1; i<numTestCases;i++){//NOTE: here the tests start at RED and NOT Ambient - I expect little different between each block's ambient response
    ...
    for (int c = 0; c < 4; c++){// For each possible colour, add the current squared error for the given test number (i)
      abs_error_vec[c] += abs(sample/5 - rgbCalibrationVals[c][i]); // This may cause overflow, if it does then we might be able to use the absolute error
    }
  }
  // Find the minimum squared error 
  int minDex = 0;
  int minVal = abs_error_vec[0];
  for (int c = 1; c<4;c++){
    if (abs_error_vec[c] < minVal){
      minDex = c;
      minVal = abs_error_vec[c];
    }
  }
